...
export function ReceptionDashboardPage() {
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-semibold text-slate-900 dark:text-slate-100">{t("Reception Dashboard")}</h1>

      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label={t("Newcomers")} value={studentsLoading ? undefined : newcomers.length} icon={UserPlus} />
        <StatCard label={t("Active Students")} value={studentsLoading ? undefined : activeStudents.length} icon={ClipboardCheck} />
      ...
      <div className="mb-8">
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{t("Recent Newcomers")}</h2>
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
          {recentNewcomers.length === 0 && (
            <p className="p-4 text-sm text-slate-500 dark:text-slate-400">{t("No newcomers registered yet.")}</p>
          )}
          {recentNewcomers.length > 0 && (
