    ...
    <>
      <Modal open={Boolean(session)} onClose={onClose} title={session.group?.name ?? "Session"}>
        <div className="space-y-3 text-sm">
          <DetailRow label="Course" value={session.course?.name ?? "-"} />
          <DetailRow label="Day" value={DAY_LABELS[session.dayOfWeek]} />
          <DetailRow label="Time" value={`${session.startTime} - ${session.endTime}`} />
          <DetailRow label="Teacher" value={session.teacher?.name ?? "Unassigned"} />
          <DetailRow label="Classroom" value={session.classroom?.name ?? "-"} />
          <DetailRow label="Branch" value={session.branch?.name ?? "-"} />
          <DetailRow label="Students" value={String(session.studentCount)} />
